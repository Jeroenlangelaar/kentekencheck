...
class ETL:
    ...
    def ingest_excel(self, source_name: str, file_name: str, content: bytes, schema_hint: dict | None = None):
        ...
        if upsert_values:
            with self._conn() as conn, conn.cursor() as cur:
                cur.execute("create temp table if not exists tmp_upsert (kenteken text, bandenmaat text, meldcode text, leasemaatschappij text, wiba_status text) on commit drop;")
